...
def run_po(stride):
    ...
    universe = pd.read_csv('data/48_Industry_Portfolios.CSV', header=6, nrows=1158)
    ...
    res_ws = portfolio_tester_rolling(universe,
                                      lambda X: l1o_cv(X, lambda X, epsilon: our_methods(X, epsilon, 'Wasserstein'), np.logspace(-10, 8, 50)), step=stride)
    ...
    for res, name in zip([res_empirical, res_linear, res_ws, res_kl, res_fr, res_nllw],
                         ['Empirical', 'Linear', 'Wasserstein', 'KL', 'Fisher-Rao', 'NLLW']):
        res['stride'] = stride
        res['method'] = name
    return [res_empirical, res_linear, res_ws, res_kl, res_fr, res_nllw]
