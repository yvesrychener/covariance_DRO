...
def kl_direct(sigma_hat, epsilon, tol=1e-5, maxit=1e5):
    ...
    sigma_p = lambda gamma: (gamma*(w**(-2))+4-np.sqrt((gamma/w)**2+8*gamma)/w)/(4*np.sqrt((gamma/w)**2+8*gamma))
    # definition of f_prime(gamma)
    f_prime = lambda gamma: (2*epsilon+p) + (np.log(sigma(gamma)/w)-sigma(gamma)/w).sum()
